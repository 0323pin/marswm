...
fn handle_window_setting<C, G, S>(
    getter: G,
    setter: S,
    controller: &C,
    window: xlib::Window,
...
impl Command {
    fn execute<C: WMController<xlib::Window>>(
        &self,
        controller: &C,
        window_result: Result<xlib::Window, String>,
        args: Args,
    ) -> Result<(), String> {
        if *self == Command::Menu {
            ...
            let result = match self {
                ...
                Command::Fullscreen(mode) => handle_window_setting(
                    C::window_is_fullscreen,
                    C::fullscreen_window,
                    controller,
                    window_result?,
                    *mode,
                ),
                Command::Pinned(mode) => handle_window_setting(
                    C::window_is_pinned,
                    C::pin_window,
                    controller,
                    window_result?,
                ...
                Command::SendToWorkspace(ws) => {
                    controller.send_window_to_workspace(window_result?, ws.index)
                }
                ...
                Command::Tiled(mode) => handle_window_setting(
                    C::window_is_tiled,
                    C::tile_window,
                    controller,
                    window_result?,
    ...
    fn menu(
        controller: &impl WMController<xlib::Window>,
        window_result: Result<xlib::Window, String>,
        args: Args,
    ) -> Result<(), String> {
        let command = match display_menu() {
            ...
            Err(e) => {
                eprintln!("Error: {}", e);
                return Err("unable to display menu".to_owned());
            }
        };
...
fn main() {
    ...
    let window_result = match args.window {
        ...
        None => controller
            .get_active_window()
            .map_err(|_| "Unable to get active window".to_string()),
    };
