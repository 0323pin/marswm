...
use crate::platforms::x11::{
    draw::widget::*, misc::atoms::X11Atom::*, misc::atoms::*, misc::window::*, misc::*, wm::*,
};
...
pub struct X11Client<A: PartialEq> {
    ...
    h: u32, // width, height
    ibw: u32,
    obw: u32,                 // inner and outer border width
...
impl<A: PartialEq> X11Client<A> {
    ...
    pub fn apply_size_hints(&mut self) {
        ...
        if let Ok((hints, _supplied)) = self.window.x11_wm_normal_hints(self.display) {
            ...
            if basew >= minw && baseh > minh {
                // required for increment calculation
                self.w -= basew;
                self.h -= baseh;
            }
...
impl<A: PartialEq> Client<A> for X11Client<A> {
    fn add_title(&mut self, font: &str, hpad: u32, vpad: u32, color: u64) -> Result<()> {
        ...
        let mut widget = X11TextWidget::new(
            self.display,
            self.frame,
            params,
            title.clone(),
    ...
    fn bind_button(&mut self, modifiers: u32, button: u32, target: ButtonTarget) {
        ...
        let window = match target {
            ...
            ButtonTarget::Root => {
                panic!("You can't bind actions to the root window through a client window")
            }
        };
    ...
    fn export_pinned(&self, state: bool, workspace_idx: Option<u32>) {
        let idx: u64 = if state {
            0xffffffff
        } else {
            workspace_idx
                .expect("Need workspace index to unpin window")
                .into()
        };
        let data = &[idx];
        self.window
            .x11_replace_property_long(self.display, NetWMDesktop, xlib::XA_CARDINAL, data);
    }
    ...
    fn hide(&mut self) {
        ...
        unsafe {
            ...
            xlib::XSelectInput(
                self.display,
                self.root,
                ra.your_event_mask & !xlib::SubstructureNotifyMask,
            );
            xlib::XSelectInput(
                self.display,
                self.frame,
                fa.your_event_mask & !xlib::StructureNotifyMask & !xlib::SubstructureNotifyMask,
            );
    ...
    fn is_centered_on_screen(&self, dimensions: Dimensions) -> bool {
        ...
        self.x == center_x - (self.w as i32 / 2) && self.y == center_y - (self.h as i32 / 2)
    }
    ...
    fn set_frame_width(&mut self, width: (u32, u32, u32, u32)) {
        ...
        self.move_resize(
            self.x - diff_west,
            self.y - diff_north,
            (self.w as i32 + diff_east + diff_west) as u32,
            (self.h as i32 + diff_north + diff_south) as u32,
        );
    }
    ...
    fn set_inner_bw(&mut self, bw: u32) {
        ...
        self.move_resize(
            self.x - diff,
            self.y - diff,
            (self.w as i32 + 2 * diff) as u32,
            (self.h as i32 + 2 * diff) as u32,
        );
    }
...
impl<A: PartialEq> Dimensioned for X11Client<A> {
    ...
    fn dimensions(&self) -> Dimensions {
        Dimensions::new(self.x, self.y, self.w, self.h)
    }
}
...
impl<A: PartialEq> X11Window for X11Client<A> {
    ...
    fn x11_read_property_long(
        ...
        self.window
            .x11_read_property_long(display, property, prop_type)
    }
    ...
    fn x11_replace_property_long(
        ...
        self.window
            .x11_replace_property_long(display, property, prop_type, data);
    }
    ...
    fn x11_set_text_list_property(
        ...
        self.window
            .x11_set_text_list_property(display, property, list);
    }
    ...
    fn x11_message(
        ...
        self.window
            .x11_message(display, msg_type, msg_format, msg_data)
    }
