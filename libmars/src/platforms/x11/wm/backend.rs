...
macro_rules! print_event {
    ($wm:expr, $event:expr) => {
        ...
        if let Some(client) = $wm.clients().find(|c| c.borrow().window() == $event.window) {
            eprintln!(
                "Received {} for window 0x{:x} (window of {})",
                event_type(&$event),
                $event.window,
                client.borrow().name()
            );
            ...
            eprintln!(
                "Received {} for window 0x{:x} (frame of {})",
                event_type(&$event),
                $event.window,
                client.borrow().name()
            );
            ...
            eprintln!(
                "Received {} for window 0x{:x} (not a client)",
                event_type(&$event),
                $event.window
            );
        }
...
impl<A: PartialEq + Default> X11Backend<A> {
    ...
    pub fn init_with_connection(display: *mut xlib::Display, name: &str) -> Result<X11Backend<A>> {
        unsafe {
            ...
            xlib::XChangeProperty(
                display,
                x11b.wmcheck_win,
                NetSupportingWMCheck.to_xlib_atom(display),
                xlib::XA_WINDOW,
                32,
                xlib::PropModeReplace,
                &x11b.wmcheck_win as *const u64 as *const u8,
                1,
            );
            xlib::XChangeProperty(
                display,
                x11b.wmcheck_win,
                NetWMName.to_xlib_atom(display),
    ...
    fn apply_dock_insets(&mut self) {
        ...
        for dock in self
            .unmanaged_clients
            .iter()
            .filter(|u| u.get_type() == UnmanagedType::Dock)
        {
            ...
            if let Some(mon) = self
                .monitors
                .iter_mut()
                .find(|m| m.contains_point(dimensions.center()))
            {
                // apply top indent
    ...
    fn handle_xevent(
        &mut self,
        wm: &mut (impl WindowManager<Self, A> + ?Sized),
        event: xlib::XEvent,
    ) {
        unsafe {
            // unsafe because of access to union field
            if self.xrandr.supported
                && event.get_type() == self.xrandr.event_base + xrandr::RRNotify
            {
        ...
        for win_type in &window_types {
            match win_type {
                NetWMWindowTypeDesktop => unsafe {
                    ...
                    self.unmanaged_clients.push(UnmanagedClient::new(
                        self.display,
                        window,
                        UnmanagedType::Desktop,
                    ));
                    return;
                },
    ...
    fn mouse_action_move(
        &mut self,
        _wm: &mut (impl WindowManager<Self, A> + ?Sized),
        client_rc: &Rc<RefCell<X11Client<A>>>,
        orig_client_pos: (i32, i32),
        _orig_client_size: (u32, u32),
        delta: (i32, i32),
    ) {
        ...
        client_rc
            .borrow_mut()
            .move_resize(dest_x, dest_y, size.0, size.1);
    }

    fn mouse_action_resize(
        &mut self,
        _wm: &mut (impl WindowManager<Self, A> + ?Sized),
        client_rc: &Rc<RefCell<X11Client<A>>>,
        _orig_client_pos: (i32, i32),
        orig_client_size: (u32, u32),
        delta: (i32, i32),
    ) {
        let dest_w = orig_client_size.0 as i32 + delta.0;
        let dest_h = orig_client_size.1 as i32 + delta.1;
        let pos = client_rc.borrow().pos();
        let dest_w: u32 = if dest_w < WINDOW_MIN_SIZE as i32 {
            WINDOW_MIN_SIZE
        } else {
            dest_w as u32
        };
        let dest_h: u32 = if dest_h < WINDOW_MIN_SIZE as i32 {
            WINDOW_MIN_SIZE
        } else {
            dest_h as u32
        };
        client_rc
            .borrow_mut()
            .move_resize(pos.0, pos.1, dest_w, dest_h);
    }
    ...
    fn on_client_message(
        ...
        if let Some(atom) = X11Atom::from_xlib_atom(self.display, event.message_type) {
            match atom {
                ...
                NetWMState => {
                    if let Some(client_rc) = Self::client_by_window(wm, event.window) {
                        if event.data.get_long(1) as u64
                            == NetWMStateFullscreen.to_xlib_atom(self.display)
                            || event.data.get_long(2) as u64
                                == NetWMStateFullscreen.to_xlib_atom(self.display)
                        {
                        ...
                        } else if event.data.get_long(1) as u64
                            == MarsWMStateTiled.to_xlib_atom(self.display)
                            || event.data.get_long(2) as u64
                                == MarsWMStateTiled.to_xlib_atom(self.display)
                        {
                            let mode = event.data.get_long(0) as u64;
    ...
    fn on_configure_request(
        &mut self,
        wm: &mut (impl WindowManager<Self, A> + ?Sized),
        event: xlib::XConfigureRequestEvent,
    ) {
        let client = wm
            .clients()
            .find(|c| c.borrow().window() == event.window)
            .cloned();
        let inner = wm
            .clients()
            .find(|c| c.borrow().window() == event.window)
            .map(|c| c.borrow().inner_dimensions());
        if let Some(client_rc) = client {
            ...
            if self
                .unmanaged_clients
                .iter()
                .any(|u| u.window() == event.window && u.get_type() == UnmanagedType::Dock)
            {
                self.apply_dock_insets();
                wm.update_monitor_config(self, self.monitors.clone());
            }
    ...
    fn on_destroy_notify(
        &mut self,
        wm: &mut (impl WindowManager<Self, A> + ?Sized),
        event: xlib::XDestroyWindowEvent,
    ) {
    ...
    fn on_key_press(
        ...
        let keysym =
            unsafe { xlib::XKeycodeToKeysym(self.display, event.keycode.try_into().unwrap(), 0) };
    ...
    fn on_unmap_notify(
        ...
        if event.event == root || client_rc.borrow().is_reparenting() {
            ...
            client_rc
                .borrow_mut()
                .x11_set_state(self.display, WITHDRAWN_STATE);
        } else {
            client_rc.borrow_mut().hide();
            self.unmanage(wm, client_rc);
        }
    ...
    fn set_supported_atoms(&mut self, supported_atoms: &[X11Atom]) {
        let atom_vec: Vec<xlib::Atom> = (*supported_atoms)
            .iter()
            .map(|a| a.to_xlib_atom(self.display))
            .collect();
        let data = atom_vec.as_slice();
        self.root
            .x11_replace_property_long(self.display, NetSupported, xlib::XA_ATOM, data)
    }
    ...
    fn client_by_frame(
        wm: &(impl WindowManager<Self, A> + ?Sized),
        frame: u64,
    ) -> Option<Rc<RefCell<X11Client<A>>>> {
        return wm
            .clients()
            .find(|c| c.borrow().frame() == frame || c.borrow().title_window() == Some(frame))
            .cloned();
    }
...
impl XRandrInfo {
    pub fn query(display: *mut xlib::Display) -> XRandrInfo {
        ...
        let supported =
            unsafe { xrandr::XRRQueryExtension(display, &mut event_base, &mut error_base) != 0 };

        XRandrInfo {
            supported,
            event_base,
            _error_base: error_base,
        }
    }
}
...
impl<A: PartialEq + Default> Backend<A> for X11Backend<A> {
    ...
    fn export_active_window(&self, client_option: &Option<Rc<RefCell<Self::Client>>>) {
        ...
        let data = &[window];
        self.root
            .x11_replace_property_long(self.display, NetActiveWindow, xlib::XA_WINDOW, data);
    }
    ...
    fn export_client_list(
        ...
        let data_stacked_vec: Vec<u64> = clients_stacked
            .iter()
        ...
        self.root
            .x11_replace_property_long(self.display, NetClientList, xlib::XA_WINDOW, data);
        self.root.x11_replace_property_long(
            self.display,
            NetClientListStacking,
            xlib::XA_WINDOW,
            data_stacked,
    ...
    fn export_workspaces(&self, mut workspaces: Vec<(String, Dimensions, Dimensions)>) {
        ...
        self.root
            .x11_set_text_list_property(self.display, NetDesktopNames, &names);

        // export workareas
        let data = workareas.as_slice();
        self.root
            .x11_replace_property_long(self.display, NetWorkarea, xlib::XA_CARDINAL, data);
    }
    ...
    fn handle_existing_windows(&mut self, wm: &mut WM<A>) {
        unsafe {
            ...
            let windows = match xlib::XQueryTree(
                self.display,
                self.root,
                &mut returned_root,
                &mut returned_parent,
                ...
                0 => Err(()),
                _ => Ok(slice::from_raw_parts(
                    top_level_windows,
                    num_top_level_windows.try_into().unwrap(),
                )),
            }
            ...
            windows
                .iter()
                .filter(|w| w.x11_is_transient_for(display).is_none())
                .filter(check_manage_window)
                .for_each(|w| self.manage(wm, *w));
            windows
                .iter()
                .filter(|w| w.x11_is_transient_for(display).is_some())
                .filter(check_manage_window)
                .for_each(|w| self.manage(wm, *w));
        ...
        unsafe {
            ...
            loop {
                ...
                if event.get_type() == xlib::MotionNotify {
                    ...
                    let delta = (
                        event.x_root - orig_pointer_pos.0,
