...
use crate::platforms::x11::misc::{
    atoms::X11Atom, atoms::X11Atom::*, send_client_message, window::X11Window,
};

pub type EventHandlerFn =
    Option<unsafe extern "C" fn(_: *mut xlib::Display, _: *mut xlib::XErrorEvent) -> c_int>;
...
impl WMController<xlib::Window> for X11Controller {
    ...
    fn count_workspaces(&self) -> Result<u32> {
        ...
        let data = self
            .root
            .x11_read_property_long(self.display, NetNumberOfDesktops, xlib::XA_CARDINAL)
            .map_err(MarsError::unknown)?;
        let ws_u64 = data.first().ok_or(MarsError::invalid_response(
            "reading property _NET_NUMBER_OF_DESKTOPS",
        ))?;
        (*ws_u64)
            .try_into()
    ...
    fn current_workspace(&self) -> Result<u32> {
        ...
        let data = self
            ...
            .map_err(MarsError::unknown)?;
        let ws_u64 = data.first().ok_or(MarsError::invalid_response(
            "reading property _NET_CURRENT_DESKTOP",
        ))?;
        (*ws_u64)
            .try_into()
            .map_err(|_| MarsError::failed_conversion(ws_u64, stringify!(u64), stringify!(u32)))
    ...
    fn get_active_window(&self) -> Result<xlib::Window> {
        ...
        let data = self
            ...
            .map_err(MarsError::unknown)?;
        data.first().copied().ok_or(MarsError::invalid_response(
            "reading property _NET_ACTIVE_WINDOW",
        ))
    }
    ...
    fn get_workspace(&self, window: xlib::Window) -> Result<u32> {
        ...
        if self.window_is_pinned(window)? {
            let data = window
                ...
                .map_err(MarsError::unknown)?;
            let ws_u64 = data.first().ok_or(MarsError::invalid_response(
                "reading property _NET_WM_DESKTOP",
            ))?;
            (*ws_u64)
                .try_into()
                .map_err(|_| MarsError::failed_conversion(ws_u64, stringify!(u64), stringify!(u32)))
    ...
    fn set_status(&self, status: String) -> Result<()> {
        let data = vec![status];
        self.root
            .x11_set_text_list_property(self.display, MarsStatus, &data);
        unsafe {
    ...
    fn window_is_pinned(&self, window: xlib::Window) -> Result<bool> {
        require_ewmh_atom(self.display, NetWMDesktop)?;
        let data = window
            .x11_read_property_long(self.display, NetWMDesktop, xlib::XA_CARDINAL)
            .map_err(MarsError::unknown)?;
        let ws_u64 = data.first().ok_or(MarsError::invalid_response(
            "reading property _NET_WM_DESKTOP",
        ))?;
...
fn require_ewmh_atom(display: *mut xlib::Display, atom: X11Atom) -> Result<()> {
    ...
    let supported = root
        .x11_read_property_long(display, NetSupported, xlib::XA_ATOM)
